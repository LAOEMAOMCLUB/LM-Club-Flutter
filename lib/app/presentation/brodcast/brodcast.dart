// BroadCastpage all BroadCast post shown here
...
class Brodcast extends StatefulWidget {
  const Brodcast({Key? key}) : super(key: key);
...
class _BrodcastState extends State<Brodcast> {
  ...
  void initState() {
    ...
    const MethodChannel('broadcast_share_channel')
        .setMethodCallHandler((methodCall) async {
      if (methodCall.method == 'onActivityResult') {
        if (methodCall.arguments == 'Success') {
          if (kDebugMode) {
            print('Debug===> shareApp---$shareApp');
          }
          if (Platform.isAndroid) {
            _broadcastBloc.sharePost(broadcastId, shareApp);
          }
        }
      }
    });
    ...
    _broadcastBloc
      ..fetchBroadcasts('', selectedDuration)
      ..shareType();
  ...
  void shareToSocialApps(
    ...
    String formattedDate = DateFormat('MM/dd/yyyy')
        .format(state.broadcastDetails[index].expiresAt!);
    String content =
        'Title: $title\n\nDescription: $description\n\nExpiry Date: $formattedDate';

    // String modeofShare = '';
// Flag to track if sharing was attempted
    ...
    if (localFiles.isNotEmpty) {
      ...
      if (result.status == ShareResultStatus.success) {
        ...
        String modeofShare = '';
        if (typeOfShareUrl.toLowerCase().contains('whatsapp')) {
          modeofShare = 'Whatsapp Share';
        } else if (typeOfShareUrl.toLowerCase().contains('instagram')) {
          modeofShare = 'Instagram Share';
        } else if (typeOfShareUrl.toLowerCase().contains('facebook')) {
          modeofShare = 'Facebook Share';
        } else if (typeOfShareUrl.toLowerCase().contains('twitter')) {
          modeofShare = 'X Share';
        } else if (typeOfShareUrl.toLowerCase().contains('teams')) {
          modeofShare = 'Microsoft Teams Share';
        } else if (typeOfShareUrl.toLowerCase().contains('snapchat')) {
          modeofShare = 'Snapchat Share';
        } else if (typeOfShareUrl.toLowerCase().contains('linkedin')) {
          modeofShare = 'LinkedIn Share';
        }
        setState(() {
          shareApp = modeofShare;
        });
        if (Platform.isIOS) {
          if (kDebugMode) {
            print('Debug===> modeofShare---$modeofShare');
          }
          _broadcastBloc.sharePost(broadcastId, modeofShare);
        }
      } else {
    ...
    return BlocProvider(
        ...
        child: BlocConsumer<BroadcastBloc, BroadcastState>(
          ...
          if (state.isSuccesful!) {
            ...
            _broadcastBloc.fetchBroadcasts('', selectedDuration);
          ...
          return Stack(children: [
            Scaffold(
                ...
                body: Column(children: [
                          ...
                          child: Container(
                            ...
                            child: Padding(
                              ...
                              child: SearchField(
                                ...
                                controller: _broadcastBloc.searchController,
                                suggestions: state.broadcastDetails
                                    .fold<List<SearchFieldListItem>>(
                                  [],
                                  (List<SearchFieldListItem> list, states) {
                                    String userName = states.userId!.username!
                                        .toLowerCase()
                                        .trim();
                                ...
                                searchInputDecoration: InputDecoration(
                                  ...
                                  suffixIcon: state.userId!.isNotEmpty
                                      ? GestureDetector(
                                          onTap: () {
                                            setState(() {
                                              _broadcastBloc.searchController
                                                  .clear();
                                              _broadcastBloc.fetchBroadcasts(
                                                  '', selectedDuration);
                                              _broadcastBloc.setUserId('');
                                            });
                                          },
                                          child: const Icon(
                                            Icons.clear,
                                ...
                                onSearchTextChanged: (x) {
                                  ...
                                  _broadcastBloc.setUserId(selectedUser);
                                  _broadcastBloc.fetchBroadcasts(
                                      selectedUser, '');
                                  return null;
                                },
                  ...
                  Flexible(
                      child: state.broadcastDetails.isEmpty
                          ? const Center(
